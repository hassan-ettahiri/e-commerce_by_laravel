        ...
        if(isset($genre)){
            if(isset($categorie)){
                if(isset($id)){
                    ...
                    return view('Frontend.itemdetail',compact('item','produits','reviews','user'));
                }
                $category = Categorie::where('categorie_nom',$categorie)->first();
                $produits = Produit::where('cate_id',$category->id)->where('produit_genre',$genre)->OrderBy('id','desc')->get();
                return view('Frontend.shop.shopbycategorie',compact('produits','genre'));
            }
            $produits = Produit::where('produit_genre',$genre)->OrderBy('id','desc')->get();
            return view('Frontend.shop.index',compact('produits','genre'));
        }
    }

    public function carte(){
        $carteitem = Carte::where('user_id',Auth::id())->OrderBy('id','desc')->get();
